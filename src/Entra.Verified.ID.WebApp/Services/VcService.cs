...
public class VcService
{
    ...
    private readonly CacheServiceWrapper _cache;
    private readonly IHostConfigurationService _httpConfigurationService;

    public VcService(HttpClient httpClient, IOptions<AppSettingsModel> appSettings, ILogger<VcService> logger,
        CacheServiceWrapper cache, VcConfigurationService settings, IOptions<EntraServicePrincipal> spSettings,
        IHostConfigurationService httpConfigurationService)
    {
    ...
    private static string IssuanceApiEndpoint =>
        VcApplicationConfiguration.VerifiableCredentialsApiEndpoint + "createIssuanceRequest";

    private static string PresentationApiEndpoint =>
        VcApplicationConfiguration.VerifiableCredentialsApiEndpoint + "createPresentationRequest";
    ...
    private async Task<string> GetAccessToken()
    {
        ...
        if (!_cache.GetCachedValue("AccessToken", out var accessToken) && accessToken != null) return accessToken;
    ...
    private void MapClaimsToVcRequest(VcIssuanceRequest request, Dictionary<string, string> claims,
        IssuanceRequest requestModel)
    {
    ...
    {
        if (string.IsNullOrEmpty(settings.Pin)) return null;
        var pinCode =
            RandomNumberGenerator.GetInt32(1,
                int.Parse("".PadRight(VcApplicationConfiguration.IssuancePinCodeLength, '9')));
